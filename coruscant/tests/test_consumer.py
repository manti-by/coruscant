...
class TestConsume:
    ...
    def test_consume__valid_message_valve_open(self, mock_logger, mock_set_gpio):
        ...
        mock_consumer = create_mock_consumer(json.dumps({"relay_id": "VALVE-OPEN", "target_state": True}).encode())
    ...
    def test_consume__valid_message_pump_off(self, mock_logger, mock_set_gpio):
        mock_set_gpio.return_value = True
        mock_consumer = create_mock_consumer(json.dumps({"relay_id": "PUMP-WF-2", "target_state": False}).encode())
    ...
    def test_consume__already_in_target_state(self, mock_logger, mock_set_gpio):
        ...
        mock_consumer = create_mock_consumer(json.dumps({"relay_id": "VALVE-CLOSED", "target_state": True}).encode())
    ...
    def test_consume__missing_relay_id(self, mock_logger):
        mock_consumer = create_mock_consumer(json.dumps({"target_state": True}).encode())
    ...
    def test_consume__unknown_relay_id(self, mock_logger):
        mock_consumer = create_mock_consumer(json.dumps({"relay_id": "unknown_relay", "target_state": True}).encode())
